...
let camera, scene, renderer, plane, controls, xrotation = -1.2, dir;
...
new IntersectionObserver((entries) => {
    postcardVisible = entries[0].isIntersecting;
}, { threshold: 0.1 }).observe(document.getElementById('postcard-section'));

new IntersectionObserver((entries, obs) => {
    if (!entries[0].isIntersecting) return;
    obs.disconnect();
    restart();
}, { rootMargin: '200px' }).observe(document.getElementById('wallmap-section'));
